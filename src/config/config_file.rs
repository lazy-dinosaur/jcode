...
impl Config {
    ...
    pub fn set_default_model(model: Option<&str>, provider: Option<&str>) -> anyhow::Result<()> {
        ...
        // M19: force an immediate reload so callers that read the global config
        // in the same turn see the new value. `save()` also invalidates and
        // notifies dependent caches, so this is normally a no-op reload check.
