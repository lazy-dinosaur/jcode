...
fn infer_persisted_session_interrupted_by_reload(session_id: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "history_reload_recovery_snapshot: fallback inspect session={} status={} last_is_user={} marker_active={} interrupted={} heuristic_last_user_reload_marker_suppressed={}",
        session_id,
        session.status.display(),
        last_is_user,
        marker_active,
        interrupted,
        last_is_user && marker_active && !interrupted
    ));
