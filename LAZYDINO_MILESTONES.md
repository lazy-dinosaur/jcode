...
| M11 | Lifecycle hook (`response.completed`, `session.stop`) 의 `decision.action` JSON 이 무시됨 — `run_blocking_lifecycle_hook` 가 exit code 만 보고 stdout JSON 파싱 안 함. 따라서 timsquad/claude-code 의 `Stop` 패턴 ("hook 이 reason 주입 → AI 다음 응답에 강제 작업") 동등 동작 불가. `tool.execute.before` deny 와 비대칭 (tool hook 만 stdout JSON 파싱) | 🟡 PARTIAL — stages 1+2 DONE on `deploy/m9-m10` commit `e345d632` (source `patch/lifecycle-hooks` commit `71101b8e`). Blocking lifecycle hooks now parse stdout decision JSON; `deny` returns a reason and `response.completed` denial is queued as a transient next-turn system reminder. Existing user-provided current-turn reminders merge with lifecycle reminders. Tests: `hooks::tests::` 17/17 passed, `lifecycle_hook` 5/5 passed. Remaining stages: (3) loop guard / hard-stop after repeated deny, (4) `session.stop` vs `client.disconnect` semantic split, (5) payload context 보강 (`last_user_message`, `recent_tool_calls`, etc.) | High (framework 게이트 강제력) |
...
| M22 | **Subagent same-round spawn defer** — main session 한 turn 안에서 subagent (`task`/`subagent` tool) 를 두 번 spawn 하면 두 번째가 즉시 실행 안 되고 background 로 deferred 됨. workaround: round 당 한 subagent 만 spawn. 이게 jcode 의 swarm/subagent dispatch 코드 버그인지, 의도된 직렬화인지 모름. 진단부터 필요 | 🔴 OPEN — 미진단. 추정 위치: `src/swarm/dispatch.rs` 또는 main turn handler. 두 가지 가능성: (a) `Mutex<TurnState>` 가 turn 종료까�� 잡혀서 같은 turn 의 두 번째 spawn 이 queue 로 밀림, (b) 의도된 single-spawn-per-turn 정책 (그렇다면 docs 갱신만). 진단: 두 번째 spawn 시점에 trace log + state inspect. 영향: parallel subagent 작업 제약. workaround 가 있어서 우선순위 낮음 | Low (workaround 있음) |
