...
| M45 | **Private `.jcode/` instruction discovery 강화** — 사용자 라이브 보고 (2026-05-13): “jcode 는 agents.md 를 크게 읽어서 따라가지 않는것같은데??”. 후속 정정: “jcode 를 프라이빗 하게 사용하고싶어서 호환성은 중요하지 않아”. 코드 확인 결과 jcode prompt path 는 `<project>/AGENTS.md`, `~/.AGENTS.md`, `<project>/.jcode/AGENTS.md`, `.jcode/harness/*.md` 중심이지만, private instruction stack 의 loaded path/order visibility, custom private globs, monorepo nested private rules 가 부족함 | 🔴 OPEN — 카드: `docs/lazydino/milestones/M45.md`. 권장 scope: Stage 1 `/info` loaded path/order 표시 + private `.jcode/*` 우선순위/ignore_project_agents 테스트 강화, Stage 2 private instruction globs, Stage 3 file-local nested private instruction injection with dedup/token budget guard. opencode/Claude compatibility 는 목표 아님 | High (private instruction-following / monorepo UX) |
...
- 🔴 **OPEN 우선순위 정렬** (Round 16 끝 시점, M28/M34 deploy 대기 제외):
  ...
  8. **M45** (High) — private `.jcode/` instruction discovery 강화.
  9. **M16** (Medium-High) — Anthropic OAuth 광고 schema 의 ToolDefinition 통일 (M13/M34 와 같은 결).
  10. **M27** (Medium) — busy-agent history fallback 두 건 (pre-existing).
  11. **M29** (Medium, 사용자 영향 0) — test 격리/순서 의존성 (CI hygiene).
  12. **M33** (Low) — 이미지 클립보드 paste UX (silent fail 시 명시).
  13. **M23** (Low) — build artifact retention.
