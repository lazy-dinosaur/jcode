...
| M9  | Hook 이중 발동 — `~/.jcode/config.toml` 이 home 하위 working_dir 에서 글로벌+project local 양쪽으로 read (자기참조 skip 없음, 코드 read 로 확정) | 🔴 OPEN — 1회 라이브 재현 + 코드 가설 확정 (T-M3 2026-05-10 13:11 UTC) | Medium-High |
...
| M12 | Anthropic OAuth provider 가 `ToolSearch` 를 AI 에게 광고하는데 dispatch 핸들러 미구현 — AI 가 호출하면 `Unknown tool: ToolSearch` 에러 | 🔴 OPEN — `src/provider/anthropic.rs:905` 에서 등록, `crates/jcode-provider-core/src/anthropic.rs:59` 주석 *"ToolSearch intentionally has no direct local analogue yet"*. 라이브 재현 (T-M8 검증 중 2026-05-10 13:25 UTC) | Medium (UX 혼란) |
| M13 | `schedule` (ScheduleWakeup) 도구 schema 가 AI 호출 형식과 불일치 — AI 가 호출하면 `missing field 'task'` 에러 | 🔴 OPEN — `src/provider/anthropic.rs:893-896` 에서 schema 정의 (`delaySeconds`/`reason`/`prompt`). AI 는 `task` 필드를 보냄. 라이브 재현 (T-M8 2026-05-10 13:27 UTC) | Medium |
| M14 | `/compact` (대화 컨텍스트 축약 명령) 동작 안 함 — 사용자 보고 (2026-05-10 13:28 UTC). 진단 미착수 | 🔴 OPEN — 증상 미확정 (어떻게 안 되는지 reproduction 필요) | Medium |
| M14a | **Emergency compaction 무한 루프** — 라이브 화면에 3회 연속 emergency compaction 관찰 (504k→20k→500k→20k→500k 패턴, 매번 dropped messages 562→564→566). 압축 직후 다시 압축 트리거 → 무한. 원인 가설: (a) system prompt + tool defs 가 거대해서 압축 불가, (b) summary 가 다시 prompt 로 expand, (c) compact loop 종료 조건 부재 | 🔴 OPEN — 라이브 재현 (2026-05-10 13:32 UTC, 사용자 화면 캡처) | **Critical (UX 파괴 + 비용 폭증)** |
| M15 | Jcode TUI 에 첨부된 이미지가 외부 jcode session (지금 우리 대화하는 session) 에는 alt-text 만 들어오고 실제 image data 가 안 옴 — 디버그 사이클 효율 영향 | 🔴 OPEN — 라이브 재현 다수 (2026-05-10 13:24~13:27 UTC, 사용자 [image 1] 첨부 매번 수신 실패) | Low-Medium (debug UX) |
