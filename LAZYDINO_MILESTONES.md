...
| M45 | **AGENTS.md / instruction discovery 가 opencode 대비 좁음** — 사용자 라이브 보고 (2026-05-13): “jcode 는 agents.md 를 크게 읽어서 따라가지 않는것같은데??”. 코드 비교 결과 jcode prompt path 는 `<project>/AGENTS.md`, `~/.AGENTS.md`, `<project>/.jcode/AGENTS.md`, `.jcode/harness/*.md` 중심. opencode 는 `AGENTS.md`/`CLAUDE.md`/`CONTEXT.md` fallback, `opencode.json.instructions` glob/URL, read tool 주변 nested instruction attach 를 지원 | 🔴 OPEN — 카드: `docs/lazydino/milestones/M45.md`. 권장 scope: Stage 1 `CLAUDE.md`/`CONTEXT.md` fallback + global compatibility + `/info` loaded path 표시, Stage 2 `[prompt] instructions` glob/path, Stage 3 file-local nested instruction injection with dedup/token budget guard | High (instruction-following / monorepo UX) |
...
- 🔴 **OPEN 우선순위 정렬** (Round 16 끝 시점, M28/M34 deploy 대기 제외):
  ...
  8. **M45** (High) — AGENTS.md / instruction discovery parity with opencode.
