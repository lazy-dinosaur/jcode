...
- 🔴 **OPEN 우선순위 정렬** (Round 16 끝 시점, M28/M34 deploy 대기 제외):
  ...
  3. ~~**M35**~~ ✅ **DONE (Round 22, 2026-05-12)** — lifecycle hook 결과의 LLM 다음 turn 자동 주입 (feedback loop 완성). `4a87de32` binary 라이브 PASS + cap=3 정확 검증 + claude-code 표준 (cap 없음, script 자율 throttle) 대비 더 안전한 default. M31 과 공통 mechanism 재사용 가능 (`ContinueImmediateWithInject` 패턴).
