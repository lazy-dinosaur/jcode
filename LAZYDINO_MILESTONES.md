...
| M18 | **SDK `Client::get_history()` 가 image bytes 명시적으로 drop** (M15 의 가장 좁은 fix candidate B) — `src/server/client_api.rs:154-159` 가 `ServerEvent::History { messages, images, .. }` 에서 `messages` 만 반환하고 `images` 는 `..` 로 버림. SDK consumer 가 image 데이터 못 받는 직접 원인. M15 의 sub-set 으로 분리 — fix 가 한 함수 추가 (~10줄) 로 해결 가능 | 🔴 OPEN — 코드 분석 완료 (T-M15 분석 산출물). Fix 방향: `pub async fn get_history_with_images() -> Result<(Vec<HistoryMessage>, Vec<RenderedImage>)>` 추가 + 기존 `get_history()` 는 backward compat 으로 유지 (또는 deprecated). 회귀 테스트: `ServerEvent::History` mock 으로 새 API 가 image bytes 보존 확인. M15 본체 (protocol-level history block 구조 변경) 보다 훨씬 작아서 먼저 처리 가능 | Low (작은 SDK 추가) |
| M19 | **Config hot-reload** — `~/.jcode/config.toml` 편집 시 `OnceLock<Config>` 가 옛 값을 캐시한 채로 process 종료까지 반영 안 됨. 장시간 떠 있는 TUI server 의 root cause 로, M9 hook fix 가 binary 는 새 것이지만 config 는 옛 것이라 효과를 못 보는 사례 발생 | ✅ **DONE (구현/테스트 완료, 통합 미적용)** — `patch/config-hot-reload` 의 `93282b44 fix(m19): hot-reload config on mtime change` + `bdf63f5c test(m19): cover hot-reload, last-good fallback and recovery`. `OnceLock<Mutex<ConfigCache>>` + 매 호출 `stat()` + 500ms debounce + `Box::leak` (기존 `&'static Config` API 보존). last-good fallback 으로 toml 파싱 깨짐에 대해 default 로 안 떨어짐. `cargo test --lib config::tests::test_m19 -- --test-threads=1` → **5 passed; 0 failed**. **deploy/m9-m10 통합은 다음 세션** (cherry-pick 시 `src/config_tests.rs` 충돌, rebase 권장 — `docs/lazydino/sessions/2026-05-11-m19-impl.md` 참조) | High (M9/M10 검증 게이트) |
| M20 | **jcode bash tool 의 2분 hard timeout** — `src/tool/bash.rs:23` 의 `DEFAULT_TIMEOUT_MS: u64 = 120000` 하드코딩 때문에 cargo test/build, 긴 sleep, stress 검증이 모두 2분에 강제 background 이동. nohup+disown 으로 process 자체는 살릴 수 있지만 호출 메시지가 끊겨 결과 폴링이 강제됨 | 🔴 OPEN (M19 작업 중 발견, 2026-05-11) — Fix 방향 두 가지: **(A) Config 노출** `[tool.bash] default_timeout_ms = 600000` 키 추가, `bash.rs` 가 읽도록. **(B) Tool input 인자** `Bash` 도구 schema 에 `timeout_ms` field, 모델이 명시적으로 길게 줄 수 있게. 권장: A+B 동시. 회귀 테스트: timeout=10000 후 sleep 30 호출 → 정확히 10초 종료 | High (모든 long-running 검증의 blocker) |
