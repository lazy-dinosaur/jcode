...
| M44 | **MCP OAuth / authenticated remote MCP support 부재** — 사용자 라이브 보고 (2026-05-13): “jcode 에 지금 mcp oauth 가 없네 맞지?”. 코드 확인 결과 `src/mcp` 에 `oauth`/`Authorization` 처리 없음, `McpServerConfig` 는 `command/args/env/shared` 기반 stdio 서버만 표현. provider OAuth 와 별개로, OAuth 를 요구하는 remote MCP server 를 직접 붙이는 흐름이 없음 | 🔴 OPEN — 카드: `docs/lazydino/milestones/M44.md`. 권장 scope: (1) MCP config 에 remote transport/url/auth block 추가, 기존 stdio backcompat 유지, (2) HTTP/SSE/streamable MCP client 구현, (3) OAuth discovery/login/refresh/token storage, (4) `mcp` tool/doctor/README UX 갱신. 주의: provider OAuth 와 섞지 말고 MCP server id scoped token storage + log redaction 필수 | High (MCP ecosystem/auth compatibility) |
| M45 | **Private `.jcode/` instruction discovery 강화** — 사용자 라이브 보고 (2026-05-13): “jcode 는 agents.md 를 크게 읽어서 따라가지 않는것같은데??”. 후속 정정: “jcode 를 프라이빗 하게 사용하고싶어서 호환성은 중요하지 않아”. 코드 확인 결과 jcode prompt path 는 `<project>/AGENTS.md`, `~/.AGENTS.md`, `<project>/.jcode/AGENTS.md`, `.jcode/harness/*.md` 중심이지만, private instruction stack 의 loaded path/order visibility, custom private globs, monorepo nested private rules 가 부족함 | ✅ **DONE** (commits `8e960ead`, `9ec3adc9`, `9ffbce0d`, priority follow-up current branch) — Stage 1 `/info`/`/context` instruction source visibility, Stage 2 `[prompt] private_instructions` globs, Stage 3 file-local nested private `.jcode/{AGENTS.md,agents.md,instructions.md,rules/*.md}` injection with current-turn dedup. Follow-up: actual system prompt now includes `AGENTS and Private Instruction Priority`, lowercase `agents.md` discovery, and docs `docs/PRIVATE_INSTRUCTIONS.md`. opencode/Claude compatibility 는 목표 아님 | — |
...
- 🔴 **OPEN 우선순위 정렬** (Round 16 끝 시점, M28/M34 deploy 대기 제외):
  ...
  7. **M44** (High) — MCP OAuth / authenticated remote MCP support.
  8. **M16** (Medium-High) — Anthropic OAuth 광고 schema 의 ToolDefinition 통일 (M13/M34 와 같은 결).
  9. **M27** (Medium) — busy-agent history fallback 두 건 (pre-existing).
  10. **M29** (Medium, 사용자 영향 0) — test 격리/순서 의존성 (CI hygiene).
  11. **M33** (Low) — 이미지 클립보드 paste UX (silent fail 시 명시).
  12. **M23** (Low) — build artifact retention.
