...
| M14a | **Emergency compaction 무한 루프** — 라이브 22회 연속 emergency compaction (사용자 13:42 UTC 보고). 504k→20k→500k 패턴 반복. retry counter `MAX_CONTEXT_LIMIT_RETRIES=5` 는 이미 존재하지만 무력화됨 — 카운터가 reset 되는 경로가 있다는 뜻 | 🔴 OPEN — 1차 진단 (2026-05-10 13:37 UTC, 15분 timebox): retry counter 는 turn 별로만 보호 + 매 turn 시작 시 0 으로 reset → 같은 messages 가 새 turn 에서 또 거대해서 무한. 깊은 작업 필요: (a) compact 직후 retry 가 또 limit 맞는 진짜 원인 (system prompt/tool defs 거대?), (b) compact 카운터가 turn 단위가 아니라 session 단위 / 시간 윈도우로 보호되어야 함, (c) emergency_truncate_with 가 호출되는지 | **Critical (UX 파괴 + 비용 폭증)** |
