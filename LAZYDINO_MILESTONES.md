...
| M11 | Lifecycle hook (`response.completed`, `session.stop`) 의 `decision.action` JSON parsing + medivance/claude-code style self-correcting stop semantics | ✅ **DONE** — stages 1+2+3+4+5+6 complete. Latest Stage 6 deploy commit `f74bffac` (source `8aa6f8c4`) makes `response.completed` deny trigger an immediate continuation turn instead of waiting for the next user prompt. Added wire-compatible `stop_hook_active: bool`, configurable `max_lifecycle_deny_streak` (env `JCODE_MAX_LIFECYCLE_DENY_STREAK` > project > global > default 3, `0` unlimited), user-turn streak reset, and streaming/non-streaming continuation handling. Tests: hooks payload golden + stop_hook_active, lifecycle deny cap/default/env/reset, config project override. | — |
