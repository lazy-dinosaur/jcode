...
| M28 | **Mermaid 렌더링 전체 미작동 — flowchart 가 텍스트 box 로만 표시됨 (이미지 변환 path 자체 disabled)** — 사용자 라이브 보고 (Round 16 Test 3, 2026-05-12). 초기 증상 진단은 "우하단 클리핑" 이었으나 실제 라이브 재현 결과 **mermaid 다이어그램이 아예 이미지로 렌더되지 않고** `┌─ mermaid` 박스 안에 raw `flowchart LR` 텍스트 그대로 표시. **근본 원인**: `crates/jcode-tui-mermaid/Cargo.toml` 의 `renderer` feature 와 `crates/jcode-tui-markdown/Cargo.toml` 의 `mermaid-renderer` feature 가 **둘 다 top-level `Cargo.toml` 에서 default 로 enable 되어 있지 않음** (`default = ["pdf"]` 만). upstream master 도 동일하게 opt-in 상태. install 스크립트 `scripts/lazydino/install-custom-jcode.sh:51` 은 `cargo build --release` 만 호출 → feature flag 없이 빌드 → 모든 `#[cfg(feature = "renderer")]` 코드 (`crates/jcode-tui-mermaid/src/lib.rs:39,84,233,259,409,618,768`, `mermaid_cache_render.rs:131,247,304,313,598` 등) 가 통째로 빠짐. **사전 결함** (M21 ver.2 회귀 아님 — backup branch `deploy-m9-m10-pre-catchup-20260511-225725Z` 도 동일 상태). 환경: Kitty terminal (`TERM=xterm-kitty`, image protocol 지원 OK), mmdc v11.12.0 + puppeteer cache 정상, 의존성 측면 문제 없음 | 🔴 OPEN — fix path: (a) **top-level `Cargo.toml` 의 `default = ["pdf"]` 를 `default = ["pdf", "mermaid-renderer"]`** 로 변경 + `mermaid-renderer = ["jcode-tui-markdown/mermaid-renderer"]` 정의 추가 (가장 단순), 또는 (b) `scripts/lazydino/install-custom-jcode.sh` 의 cargo build 명령에 `--features mermaid-renderer` 추가 (스크립트 한 줄 수정), (c) 라이브 검증: 빌드 후 같은 mermaid 프롬프트 재현 → 이미지로 렌더되는지 + 우하단 클리핑 (원래 M28 의도) 도 여전히 발생하는지 확인. 우선순위 High (사용자 기대 기능 완전 미작동) | High (feature 전체 사용 불가) |
| M29 | **Test 격리/순서 의존성 오염 — 26 + 2 + 1 = 29 fail** — `cargo test --workspace` 시 `jcode --lib` 26 fail + abort, `jcode --test e2e` 2 fail (`burst_spawn::*` deadline elapsed), `jcode-tui-mermaid --lib` 1 fail (`test_mermaid_render_queue_caches_completed_render` background render 10s timeout). 각 fail test 를 **단독 실행하면 모두 PASS** 확인됨 (예: `cargo test bus::tests::models_updated_publishes_are_coalesced` ok, `auth::tests::full_and_fast_auth_status_match_for_shared_probe_fields` ok). 즉 **테스트 순서/parallel 시 글로벌 state 오염**. 의심 원인: M21 ver.2 합성에서 새로 추가된 `notify_config_reloaded()` 가 `crate::bus::Bus::global().publish_models_updated()` 호출 → Bus broadcast subscriber 들이 다른 test 의 상태 오염. fail test 들이 모두 config/auth/provider/openrouter 등 **글로벌 cache 의존 영역** 인 게 일관됨. **사용자 영향: 0** — release binary 동작 영향 없음, CI 만 noisy | 🔴 OPEN — 진단 단계: (1) `Bus::global()` subscriber drop 시점 확인 (test 격리 위해 per-test reset 필요할 수 있음), (2) `notify_config_reloaded` 가 cfg(test) 시 publish 생략하도록 가드 추가 검토, (3) git bisect 로 fail 시작 commit 확정. 우선순위 Medium (deploy blocker 아님, CI hygiene 만) | Medium (사용자 영향 0, CI 만) |
...
| M31 | **Background tool 결과의 LLM 자동 주입 (auto-enqueue)** — 현재 `nohup ... &` 같은 bg task 가 끝나면 `event_history`에 `completed` 만 기록되고 사용자 알림 (M30) 도 별개. LLM agent 가 그 결과를 다음 turn input 으로 자동으로 받으려면 매번 `Bash` (sync) 또는 `bg action="wait"` 또는 사용자가 "어떻게 됐어?" 라고 물어야 함. **자동화 누락**: bg task `completed` 시점에 그 stdout 을 **`tool_result` 처럼 LLM 컨텍스트에 append 하고 다음 inference turn 을 자동 trigger** 하는 path 가 없음. 사용자 round 14 명시: "릴리즈하고 bg 에서 돌리는 툴 결과를 너가 바로 받아서 결과로 사용해야 하는데 그것도 안 됐고" | 🔴 OPEN — 설계 옵션: (a) bg `notify=true` flag 확장하여 `auto_inject=true` 시 stdout/exit_code 를 자동으로 새 user message ("[bg_complete: task_id, output: ...]") 로 inject + turn trigger, (b) opt-in slash command `/bg-wait-all` 로 모든 진행 중 bg 의 완료까지 LLM block, (c) tool API 에 `BgTask` 타입 새로 추가하여 LLM 이 await 의도를 명시. 우선순위 High (autonomy 핵심) | High (autonomy 직접) |
| M32 | **Assistant streaming events (`TextDelta`/`ToolStart`/`ToolDone`/`MessageEnd`/`Done`) 의 sibling client fanout 누락 — bg wake/sibling attach 환경에서 응답이 보이지 않는 증상** — 사용자 라이브 보고 (2026-05-11 round 14): Alt+B 로 bg 전환된 task 가 끝나고 system-reminder 가 들어와 assistant continuation turn 이 정상 trigger 되었으나 **UI 에 응답이 렌더링되지 않음**. 백엔드 흐름은 정상 (`bg complete` → `system-reminder` insert → assistant turn → text generated) 인데 frontend 가 못 받음. 근본 원인: `src/agent/turn_streaming_mpsc.rs:260` (그리고 `turn_streaming_broadcast.rs:261,270,288`) 에서 LLM streaming 이 단일 `event_tx: mpsc::UnboundedSender<ServerEvent>` 로만 보냄 — 즉 **agent turn 의 모든 ServerEvent (TextDelta/ToolStart/ToolExec/ToolDone/GeneratedImage/TokenUsage/MessageEnd/Done 등) 가 "이 turn 을 시작한 single connection" 한 곳에만 가고, 같은 session 에 attach 한 sibling client 의 `event_txs` map 에는 broadcast 안 됨**. M11 (multi-attach) / M15 (user-message fanout) 가 자가-attach + user message 까지만 해결, **assistant turn streaming** 은 안 했음. 사용자 영향 직접: ambient/swarm/comm_task 등 bg-trigger turn 의 응답을 다른 attached client 가 못 봄 | 🔴 OPEN — 코드 fix path: agent turn 에 전달하는 `event_tx: mpsc::UnboundedSender<ServerEvent>` 를 **`SessionEventDispatcher`** (multi-sender) 로 교체 또는 send 호출마다 `fanout_session_event(swarm_members, session_id, event)` 로 변경. fanout 대상 events: `TextDelta`, `TextReplace`, `ToolStart`, `ToolInput`, `ToolExec`, `ToolDone`, `GeneratedImage`, `BatchProgress`, `TokenUsage`, `MessageEnd`, `UpstreamProvider`, `Interrupted`, `Done`, `Error` (`server_events.rs:19-36` 의 list). 우선순위 High (multi-client + bg-trigger + ambient wake 셋 다 깨짐). **Round 16 Test 6 추가 재현** (2026-05-12): ambient wake (schedule 1분 후) 가 due 되어 assistant 응답을 정상 생성했으나 **사용자 UI 에 안 보이는** 케이스 또 발생. 사용자 인용: "근데 그게 눈에 안보였는데??". 이로써 M32 는 (1) bg complete wake, (2) ambient schedule wake — 두 종류의 server-initiated turn 모두에서 UI 누락 재현 확정. | High (사용자 라이브 회귀) |
...
- ✅ M21 ver.2 (round 15) — 130 lazydino + 178 새 upstream commit squash-rebase, 174 file / +20581/-723 lines, 5-path sha256 `665bddf93f...` 동기, fork push 완료.
- 🔴 **OPEN 우선순위 정렬**:
  1. **M32** (High, 사용자 라이브 회귀) — assistant streaming 이 sibling client 에 broadcast 안 됨. bg wake/multi-attach 둘 다 영향.
  2. **M31** (High, autonomy 핵심) — bg tool 결과의 LLM 자동 주입 없음.
  3. **M30** (Medium-High) — bg `notify=true` 가 실제 wake 전달 안 함.
  4. **M28** (Medium-High) — Mermaid 우하단 클리핑.
  5. **M27** (Medium) — busy-agent history fallback 두 건 (pre-existing).
  6. **M24** (Medium-High) — 일반 tool round-내 병렬 실행 (M22 후속).
  7. **M17** (High, 사용자 워크플로우) — main↔swarm queue 라우팅.
  8. **M16** (Medium-High) — Anthropic OAuth 광고 schema 의 ToolDefinition 통일.
  9. **M29** (Medium, 사용자 영향 0) — test 격리/순서 의존성 (CI hygiene).
  10. **M23** (Low) — build artifact retention.
  11. **M33** (Low) — 이미지 클립보드 paste UX (silent fail 시 명시).
  12. **M34** (Medium-High) — schedule tool 광고 schema vs deserialize struct 필드 이름 mismatch.
  13. **M35** (High, autonomy + hook 활용도 직결) — lifecycle hook 결과의 LLM 다음 turn 자동 주입 (feedback loop 완성).
