...
///
/// Each compaction event is recorded as a single `StoredCompactionTurn`,
/// stored on the session in addition to the legacy `compaction` field. The
/// turn references a marker message (`marker_message_id`, a user-role
/// `StoredMessage` with no provider-visible content) and a summary message
/// (`summary_message_id`, an assistant-role `StoredMessage` carrying the
/// summary text). When a future compaction runs, it links to the previous
/// summary via `previous_summary_id` so consumers can rebuild the anchored
/// chain.
///
/// Sidecar metadata pattern (Option A in M48.md): the marker/summary
/// messages themselves stay as normal `StoredMessage`s in the messages
/// vector. This struct only carries the *relationships* and provider-payload
/// hints (`tail_start_id`, redaction flags). That way:
/// - Provider conversion remains unchanged when the legacy `compaction`
///   field is used.
/// - Exports / conversation search can render the summary text from the
///   referenced message but skip the marker (because it has empty visible
///   content).
/// - Future migration to durable-only mode just stops writing the legacy
///   `compaction` field; the messages themselves stay where they are.
///
/// Backfill: when a session is loaded with a legacy `compaction` value but
/// no `compaction_turns`, a synthetic `StoredCompactionTurn` is generated
/// (see `compaction_turns_backfilled_from_legacy`). The legacy field is
/// preserved untouched so existing provider conversion paths keep working.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct StoredCompactionTurn {
    /// Stable identifier for this compaction turn within the session.
    pub id: String,

    /// `StoredMessage.id` of the user-role marker message.
    ///
    /// May be empty for legacy-backfilled turns where no marker message is
    /// available (the legacy schema only persisted summary text, never a
    /// dedicated marker).
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub marker_message_id: String,

    /// `StoredMessage.id` of the assistant-role summary message.
    ///
    /// May be empty for legacy-backfilled turns; consumers should fall back
    /// to reading `Session.compaction.summary_text` in that case.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub summary_message_id: String,

    /// True when this compaction was triggered automatically (background
    /// proactive / overflow path), false when the user invoked `/compact`.
    #[serde(default)]
    pub auto: bool,

    /// True when this compaction was forced by a context overflow error
    /// from the provider. The replay path uses this to rebuild the last
    /// user message after the summary lands.
    #[serde(default)]
    pub overflow: bool,

    /// `StoredMessage.id` of the first message that is preserved verbatim
    /// in the recent-tail after this compaction. `None` means the tail is
    /// empty and the provider only sees the summary plus future messages.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tail_start_id: Option<String>,

    /// `StoredCompactionTurn.id` of the prior anchored summary, if any.
    /// Allows consumers to walk the summary chain for diagnostics or for
    /// future "anchored summary update" prompts that fold previous_summary
    /// into the next one.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub previous_summary_id: Option<String>,

    /// `StoredMessage.id`s that this summary represents. Used by exports
    /// and search to redact compacted regions when the summary alone is
    /// shown. Empty for legacy-backfilled turns.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub summary_of_message_ids: Vec<String>,

    /// M48-C5c: `StoredMessage.id` of the synthetic user message appended after
    /// an overflow compaction when auto-continue is enabled. This is either a
    /// replay of the last safe user message (with media rewritten to text
    /// labels) or a generic continue prompt.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub replay_message_id: Option<String>,

    /// M48-C5c: distinguishes synthetic replay from synthetic continue prompts.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub replay_kind: Option<StoredCompactionReplayKind>,

    /// True when this turn was generated by backfilling the legacy
    /// `Session.compaction` field on load. Tells downstream code that
    /// `marker_message_id` / `summary_message_id` may be empty and that
    /// the canonical summary lives in `Session.compaction.summary_text`.
    #[serde(default, skip_serializing_if = "is_false")]
    pub backfilled_from_legacy: bool,

    /// UTC timestamp at which the compaction was recorded.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum StoredCompactionReplayKind {
    Replay,
    Continue,
}

impl StoredCompactionTurn {
    /// True when this turn was reconstructed from a legacy `compaction`
    /// field and has no dedicated marker/summary messages.
    pub fn is_legacy_backfill(&self) -> bool {
        self.backfilled_from_legacy
    }

    /// True when the turn references both a marker and a summary message.
    pub fn has_durable_messages(&self) -> bool {
        !self.marker_message_id.is_empty() && !self.summary_message_id.is_empty()
    }
}
